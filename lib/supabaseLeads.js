...
// IMPORTANT: Supabase's PostgREST API caps a single request at 1000 rows by
// default, regardless of the `limit` query param — so this must page through
// results using the Range header until a page comes back under the page size.
async function getCachedLeads() {
  ...
  while (true) {
    ...
    const res = await fetch(`${LEADS_CACHE_URL}?select=*&order=created_time.desc`, {
